...
//
// ชุด regression เชิงโครงสร้างของ parser — สังเคราะห์ขึ้นทั้งหมด
//
// ประโยคชุดนี้ผ่านการตรวจโดยเจ้าของภาษาในกลุ่มเป้าหมาย จึงใช้เป็น
// regression gate 98/95/80 เพื่อกันของที่เคยทำได้พังในรอบหลังได้
// แต่ห้ามเรียกผลจากชุดนี้ว่า accuracy กับผู้ใช้จริง
// accuracy จริงต้องวัดใหม่เมื่อมีข้อความ verbatim จากผู้ใช้จริง >= 50 ประโยค
//
...
const parserEdgeCases = <ParserCase>[
  ...
  ParserCase(
    ...
    note: 'ย้ายจากชุดสังเคราะห์เดิม · เว้นวรรคระหว่างตัวเลขกับหน่วยจำนวน',
  ),
  ...
  ParserCase(
    ...
    type: ExpectedType.saving,
    contextDependent: true,
    note: 'ย้ายจากชุดสังเคราะห์เดิม · วลีกริยาเต็มของการออมเข้ากระปุก',
  ),
  ...
  ParserCase(
    ...
    type: ExpectedType.income,
    category: 'รายได้เสริม',
    note: 'ย้ายจากชุดสังเคราะห์เดิม · รายรับที่ไม่เว้นวรรคก่อนจำนวนเงิน',
  ),
  ParserCase(
    group: 'E-income',
    input: 'แม่ให้ 500',
  ...
  ParserCase(
    ...
    type: ExpectedType.expense,
    category: 'อาหาร',
    note: 'ย้ายจากชุดสังเคราะห์เดิม · ชื่ออาหารติดกับจำนวนโดยไม่เว้นวรรค',
  ),
  ParserCase(
    group: 'F-category',
    input: '7-11 89',
  ...
  ParserCase(
    ...
    type: ExpectedType.expense,
    category: 'เดินทาง',
    note: 'ย้ายจากชุดสังเคราะห์เดิม · คำเดินทางติดกับจำนวนโดยไม่เว้นวรรค',
  ),
  ParserCase(
    group: 'F-category',
    input: 'วิน 20',
  ...
  ParserCase(
    ...
    type: ExpectedType.expense,
    category: 'การศึกษา',
    note: 'ย้ายจากชุดสังเคราะห์เดิม · จำนวนหลักหมื่นที่ไม่ใส่ comma',
  ),
  ParserCase(
    group: 'F-category',
    input: 'ค่าส่ง 40',
  ...
  ParserCase(
    ...
    input: 'โอน500',
    tier: ExpectedTier.low,
    note: 'ย้ายจากชุดสังเคราะห์เดิม · ความกำกวมต้องคงเดิมแม้ไม่เว้นวรรค',
  ),
  ParserCase(
    group: 'G-ambiguous',
    input: 'ยืมเพื่อน 200',
  ...
  ParserCase(
    ...
    category: 'ที่พัก',
    note:
        'ย้ายจากชุดสังเคราะห์เดิม · ประโยค recurring ที่พิมพ์ติดกันทั้งบรรทัด',
  ),
